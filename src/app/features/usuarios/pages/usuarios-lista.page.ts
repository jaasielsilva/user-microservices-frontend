...
/**
 * "Novo" aponta pra /criar-conta (fluxo de registro no auth-service),
 * não pra uma rota interna deste feature — criar usuário não é um
 * CRUD direto aqui, ver usuario.service.ts.
 */
@Component({
  ...
  template: `
    <div class="barra-topo">
      ...
      <div style="display: flex; gap: 8px; align-items: center">
        ...
        />
        @if (ehAdmin()) {
          <a class="btn" routerLink="/criar-conta">Novo</a>
        }
      </div>
    </div>
...
export class UsuariosListaPage {
  ...
  protected async excluir(item: Usuario): Promise<void> {
    const confirmado = await this.confirmacao.perguntar(
      'Excluir usuário?',
      `"${item.nome}" sai da listagem. A credencial de login não é removida por este fluxo.`,
